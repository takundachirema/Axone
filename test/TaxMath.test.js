...
contract("HarbergerTaxManager: Math 🔬", ([surrogateRegistry, contractOwner, random]) => {
    beforeEach(async function () {
        this.harbergerTaxManager = await HarbergerTaxManager.new({
            from: contractOwner
        })

        this.harbergerTaxManager.initialize(surrogateRegistry, {
            from: contractOwner
        })
    });
    context("Helper Maths 🦾", function () {
        it(`Exponentual calculation loop`, async function () {
            ...
            for (let percent = 0; percent < 10; percent++) {
                ...
                let fixedPoint = await this.harbergerTaxManager.optimalExp.call(FIXED_1.times(x).toFixed(0));
        ...
        it("Can correctly find capatalization function", async function () {
            ...
            let contractValue = await this.harbergerTaxManager.capFunction.call(borrowRatePerBLock, 0, secondsPerYear);
        ...
        it("Can correctly calculate future value of NACC position", async function () {
            ...
            let contractValue = await this.harbergerTaxManager.futureValue.call(nominal.toString(), borrowRatePerBLock, 0, secondsPerYear);
            let ratio = new BigNumber(expectedValue).dividedBy(contractValue);
